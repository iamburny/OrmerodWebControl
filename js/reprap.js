    ...
    if (!status || !polling) {
        ...
        if (status.status == "S") {
            ...
            if (printStartTime && objTotalFilament > 0) {
                setProgress(Math.floor((objUsedFilament.reduce(function (a, b) {
                    return a + b;
                }, 0) / objTotalFilament) * 100), 'print', currentLayer, layerCount);
                estEndTime();
...
function getFilamentUsed() {
    ...
    return objUsedFilament.reduce(function (a, b) {
        return a + b;
    }, 0);
}
...
function layers(layer) {
    ...
    if ((layer === 1 || layer === 2) && !printStartTime) {
        ...
        layerData.push(utime);
        filamentData.push(startingFilamentPos.reduce(function (a, b) {
            return a + b;
        }, 0));
    }
    if (printStartTime) {
...
function setProgress(percent, bar, layer, layers) {
    ...
    if (bar == 'print') {
        ...
        if (objTotalFilament > 0) {
            var currentTotal = currentFilamentPos.reduce(function (a, b) {
                return a + b;
            }, 0);
            var startingTotal = startingFilamentPos.reduce(function (a, b) {
                return a + b;
            }, 0);
